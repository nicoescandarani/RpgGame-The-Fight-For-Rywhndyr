...
let GameManager = {
    ...
    resetPlayer: function(type, race) {
        ...
        switch (type) {
            ...
            case 'Ortanna':
                hero = new Hero(type, race, 120, 0, 180, 120, 100);
                break;
    ...
    setFight: function() {
        ...
        const enemy0 = new Enemy('Vulur', ORC, 100, 0, 60, 60, 90, 'Forest', VULUR_STORY);
        const enemy1 = new Enemy('Wndorf', GOBLIN, 90, 10, 120, 80, 100, 'City', WNDORF_STORY);
        const enemy2 = new Enemy('Dulkar', HUMAN, 60, 0, 140, 40, 100, 'Village', DULKAR_STORY);
        const enemy3 = new Enemy('Osgrwd', SKIN_SHIFTER, 160, 70, 200, 80, 150, 'CityRuins', OSGRWD_STORY);
        ...
        const currentStory = document.querySelector('.storyWrapper');
        const storyContainer = document.querySelector('.currentStory');
        let interface = document.querySelector('.interface');
        let width = window.innerWidth;
        ...
        if (localStorage.getItem('countEnemies').length == enemies.length) {
            ...
            // ! Generate Hero card.
            interface.innerHTML = '<div class="winner-wrapper"><div class="btn-wrapper msg winner-message"><div class="pilar"></div><p class="btn message success">Congralutions sir, Rywhndyr is yours!</p><div class="pilar"></div></div><img src="Assets/' + hero.type + '.png" alt="' + hero.type + '" class="hero-img-success"><div class="btn-wrapper"><div class="pilar"></div><button onClick="GameManager.reloadPage()" class="btn btn-success">Play again!</button><div class="pilar"></div></div></div>';
        } else {
